...
list.push_back(node_list.at(i));
	...
	void preOrderCount(int* count, Node* node)
	{
		*count = *count + 1;
		...
		for (int i = 0; i < node->getChildren().size(); i++)
		{
			preOrderCount(count, node->getChildren().at(i));
		}
	}
	void seedLess(Node* seed)
	{
		if (seed->getChildren().empty())
		{
			cout << seed->getData() << " ";
		}
		for (int i = 0; i < seed->getChildren().size(); i++)
		{
			seedLess(seed->getChildren().at(i));
		}
	}
	Node* atData(int data)
	{
		for (Node* result : node_list)
		{
...
int main()
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		Node* seedNode = tree->atData(seed);
		if(seedNode != nullptr)
			tree->seedLess(seedNode);
		cout << endl;
		delete tree;
